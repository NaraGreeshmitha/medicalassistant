...
model = load_model('models/medical_diagnosis_model.h5')
with open('models/label_encoder.pkl', 'rb') as f:
    label_encoder = pickle.load(f)
with open('models/symptom_index.pkl', 'rb') as f:
    symptom_index = pickle.load(f)

# Function to encode symptoms into a format suitable for the model
def encode_symptoms(symptoms, symptom_index):
    encoding = np.zeros(len(symptom_index))
    for symptom in symptoms.split(', '):  # Make sure to split symptoms as expected by your model
        if symptom in symptom_index:
            encoding[symptom_index[symptom]] = 1
    return encoding

# Function to predict the medical condition based on symptoms
def predict_condition(user_symptoms, model, symptom_index, label_encoder):
    encoded_symptoms = encode_symptoms(user_symptoms, symptom_index)
    prediction = model.predict(np.array([encoded_symptoms]))
    predicted_index = np.argmax(prediction)
    predicted_condition = label_encoder.inverse_transform([predicted_index])[0]
...
def index():
    if request.method == 'POST':
        user_symptoms = request.form['symptoms']
        condition = predict_condition(user_symptoms, model, symptom_index, label_encoder)
        recommended_doctors = recommend_doctors(condition)
        ai_advice = get_ai_response(user_symptoms)
        advice_paragraph = convert_to_paragraph(ai_advice)
        return render_template('results.html', doctors=recommended_doctors, condition=condition, query=user_symptoms,advice=advice_paragraph)
...
@app.route('/book/<doctor_name>', methods=['GET'])
def book_appointment_form(doctor_name):
    doctor = mongo.db.newdoc.find_one({"Name": doctor_name})
    if not doctor:
        return "Doctor not found", 404
    # Pass doctor availability to the template if needed
    return render_template('book_appointment.html', doctor=doctor)
...
def book_appointment(doctor_name):
    ...
    mongo.db.newdoc.update_one({"Name": doctor_name}, {"$push": {"Appointments": new_appointment}})
